...
class cmb_Meta_Box {
	...
	public function register_scripts() {
		...
		if ( 3.5 <= $wp_version ) {
			...
			if ( ! is_admin() ) {
				...
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear'         => __( 'Clear', 'unconditional' ),
					'defaultString' => __( 'Default', 'unconditional' ),
					'pick'          => __( 'Select Color', 'unconditional' ),
					'current'       => __( 'Current Color', 'unconditional' ),
				) );
			}
		} else {
		...
		wp_localize_script( 'cmb-scripts', 'cmb_l10', apply_filters( 'cmb_localized_data', array(
			...
			'upload_file'     => __( 'Use this file', 'unconditional' ),
			'remove_image'    => __( 'Remove Image', 'unconditional' ),
			'remove_file'     => __( 'Remove', 'unconditional' ),
			'file'            => __( 'File:', 'unconditional' ),
			'download'        => __( 'Download', 'unconditional' ),
			'ajaxurl'         => admin_url( '/admin-ajax.php' ),
			'up_arrow'        => '[ ↑ ]&nbsp;',
			'down_arrow'      => '&nbsp;[ ↓ ]',
			'check_toggle'    => __( 'Select / Deselect All', 'unconditional' ),
		) ) );
...
function cmb_metabox_form( $meta_box, $object_id, $echo = true ) {
	...
	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, __( 'Save', 'unconditional' ) );
